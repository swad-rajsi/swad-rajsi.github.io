---
layout: recipe
title: "Punjabi Rajma Masala"
title_hi: "पंजाबी राजमा मसाला"
lang_default: en
show_language_change_btn: true
published: true
prep_time: 8 hours
cook_time: 35 minutes
total_time: 515   # in minutes for filter
course: Main, Lunch, Dinner
flavour: Warm, Savory, Spicy
dietary_pattern: Lacto-veg, Veg, Includes dairy, No eggs, No meat
food_type: Punjabi
serving_original: 4
serving: 4
permalink: /recipes/punjabi-rajma-masala/
image: /assets/images/recipes/punjabi_rajma_masala.jpg
ingredients_en:
  - heading: Soaking & Pressure Cooking
    items:
      - qty: 1
        unit: cup
        name: Rajma (Kidney Beans), soaked overnight (8 hours)
      - qty: 1
        unit: teaspoon
        name: Salt
      - qty: 3
        unit: cup
        name: Water

  - heading: Curry Preparation
    items:
      - qty: 1
        unit: tablespoon
        name: Ghee or Vegetable Oil
      - qty: 1
        unit: teaspoon
        name: Cumin seeds
      - qty: 1
        unit: pinch
        name: Asafoetida (Heeng)
        scale: false
      - qty: 1
        unit:
        name: Black Cardamom
      - qty: 1
        unit: inch
        name: Cinnamon stick
      - qty: 4
        unit:
        name: Cloves
      - qty: 1
        unit:
        name: Bay Leaf
      - qty: 1
        unit: tablespoon
        name: Ginger Garlic Paste
      - qty: 2
        unit:
        name: Onion, chopped (total 120 g)
      - qty: 1/4
        unit: teaspoon
        name: Turmeric powder
      - qty: 1
        unit: teaspoon
        name: Kashmiri red chilli powder
      - qty: 1
        unit: teaspoon
        name: Coriander powder
      - qty: 1/2
        unit: teaspoon
        name: Cumin powder
      - qty: 4
        unit:
        name: Tomatoes, chopped or paste (total 350 g)
      - qty: 1
        unit:
        name: Green chilli, slit
      - qty: 1
        unit: teaspoon
        name: Salt (or to taste)
      - qty: 1/2
        unit: teaspoon
        name: Garam Masala
        link: https://en.wikipedia.org/wiki/Garam_masala
      - qty: 1.5
        unit: tablespoon
        name: Dried fenugreek leaves (Kasuri Methi), crushed

  - heading: Garnish
    items:
      - qty:
        unit:
        name: Ginger slices (julienne)
        scale: false
      - qty: 1
        unit: tablespoon
        name: Coriander leaves
      - qty: 2
        unit: tablespoon
        name: Fresh cream


ingredients_hi:
  - heading: भिगोना और प्रेशर कुकिंग
    items:
      - qty: 1
        unit: कप
        name: राजमा (किडनी बीन्स), 8 घंटे भिगोया हुआ
      - qty: 1
        unit: टीस्पून
        name: नमक
      - qty: 3
        unit: कप
        name: पानी

  - heading: ग्रेवी की तैयारी
    items:
      - qty: 1
        unit: टेबलस्पून
        name: घी या तेल
      - qty: 1
        unit: टीस्पून
        name: जीरा
      - qty: 1
        unit: चुटकी
        name: हींग
        scale: false
      - qty: 1
        unit:
        name: बड़ी इलायची
      - qty: 1
        unit: इंच
        name: दालचीनी
      - qty: 4
        unit:
        name: लौंग
      - qty: 1
        unit:
        name: तेज पत्ता
      - qty: 1
        unit: टेबलस्पून
        name: अदरक लहसुन पेस्ट
      - qty: 2
        unit:
        name: प्याज, बारीक कटा हुआ (कुल 120 ग्राम)
      - qty: 1/4
        unit: टीस्पून
        name: हल्दी
      - qty: 1
        unit: टीस्पून
        name: कश्मीरी लाल मिर्च पाउडर
      - qty: 1
        unit: टीस्पून
        name: धनिया पाउडर
      - qty: 1/2
        unit: टीस्पून
        name: जीरा पाउडर
      - qty: 4
        unit:
        name: टमाटर, कटे हुए या पेस्ट (कुल 350 ग्राम)
      - qty: 1
        unit:
        name: हरी मिर्च, चीरी हुई
      - qty: 1
        unit: टीस्पून
        name: नमक (या स्वाद अनुसार)
      - qty: 1/2
        unit: टीस्पून
        name: गरम मसाला
      - qty: 1.5
        unit: टेबलस्पून
        name: कसूरी मेथी, मसलकर

  - heading: सजावट
    items:
      - qty:
        unit:
        name: अदरक के लच्छे
        scale: false
      - qty: 1
        unit: टेबलस्पून
        name: हरा धनिया
      - qty: 2
        unit: टेबलस्पून
        name: फ्रेश क्रीम


steps_en:
  - title: Boil Rajma
    actions:
      - Wash and rinse rajma 2–3 times.
      - Soak in enough water for 7–8 hours or overnight.
      - Once soaked, discard soaking water and rinse again.
      - Pressure cook with water and salt for 5–6 whistles or 12–15 minutes in Instant Pot.
      - Rajma should be soft but hold shape. Keep aside.

  - title: Prepare Curry
    actions:
      - Heat ghee/oil in a pan on medium flame.
      - Add cumin seeds, asafoetida (heeng), whole spices (ingredients b to g) and sauté briefly.
      - Add ginger garlic paste and cook for 1 minute.
      - Add chopped onions and sauté till golden brown.
      - Add powdered spices (ingredients j to m) and sauté till aromatic.
      - Add tomatoes and cook till oil separates.
      - Add boiled rajma, green chilli and water as needed.
      - Cover and cook for 15 minutes for rajma to absorb flavour.
      - Add garam masala and dried fenugreek leaves(kasuri methi), mix well.
      - Garnish with ginger, coriander and cream before serving.
      - It will get thick when cooled down. Adjust consistency with water and salt/ chilli if required.


steps_hi:
  - title: राजमा उबालें
    actions:
      - राजमा को 2–3 बार धो लें।
      - 7–8 घंटे या रात भर भिगो दें।
      - भिगोने के बाद तैयार हो जाने पर, पानी निकालकर फिर से धो लें।
      - प्रेशर कुकर में पानी और नमक डालकर 5–6 सीटी तक पकाएँ।
      - राजमा नरम हो लेकिन आकार बना रहे।

  - title: ग्रेवी तैयार करें
    actions:
      - कढ़ाही में घी/तेल गरम करें।
      - जीरा, हींग और साबुत मसाले (सामग्री b से g) डालें।
      - अदरक लहसुन पेस्ट डालकर भूनें।
      - बारीक कटा हुआ प्याज डालकर सुनहरा होने तक भूनें।
      - सूखे मसाले (सामग्री j से m) डालें और खुशबू आने तक पकाएँ।
      - टमाटर डालें और तेल अलग होने तक पकाएँ।
      - उबला राजमा, हरी मिर्च और पानी डालें।
      - राजमा को स्वाद सोखने के लिए ढककर 15 मिनट पकाएँ।
      - गरम मसाला और कसूरी मेथी मिलाएँ।
      - अदरक, हरा धनिया और क्रीम से सजाएँ।
      - राजमा ठंडा होने पर गाढ़ा हो जाएगा। गाढ़ापन ठीक करने के लिए पानी डालें और आवश्यकतानुसार नमक/मिर्च डालें।


description_en: |
  Rajma Masala is a classic vegetarian Punjabi curry made with red kidney beans simmered in a rich, spiced tomato gravy. It tastes best when served hot with steamed rice or jeera rice.

description_hi: |
  राजमा मसाला एक पारंपरिक शाकाहारी पंजाबी व्यंजन है जिसमें राजमा को मसालेदार टमाटर की ग्रेवी में पकाया जाता है। यह चावल या जीरा राइस के साथ सबसे अच्छा लगता है।
---